...
import { DOSE_UNIT_LABELS, MEDICATION_DOSAGE_FORM } from "@/constants/medication"
import { IRES_Medication } from "@/types/api"
import { DosageForm } from "@/types/common"
import { IEvent } from "@/types/schedule"
import dayjs from "dayjs"

export const evaluteStatus = ({ intakenTime, scheduledTime }: Pick<IEvent, 'scheduledTime' | 'intakenTime'>) => {
    const now = dayjs()
    ...
    // 已到時間，且仍在可服用時間內
    else if (now.isBefore(lastIntakeTime) || now.isSame(lastIntakeTime)) {
        isOperable = true
    }
    // 已逾時
    else {
...
export const evaluateDosageFormIcon = ({ dosageForm }: { dosageForm: IRES_Medication['dosageForm'] | null }) => {
...
export const evaluateLabel = ({
    ...
    return {
        label: list.join(' '),
        icon: evaluateDosageFormIcon({ dosageForm: medicationDosageForm })
    }
}