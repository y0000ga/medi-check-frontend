...
const Screen = () => {
  ...
  const filteredHistories = useMemo(() => {
    if (viewerMode === 'self') {
      return histories.filter((history) => history.patientId === selfPatient?.id)
    }

    if (selectedPatientId) {
      return histories.filter((history) => history.patientId === selectedPatientId)
    }

    const carePatientIds = new Set(carePatients.map((item) => item.patientId))
    return histories.filter((history) => carePatientIds.has(history.patientId))
  }, [carePatients, histories, selectedPatientId, selfPatient?.id, viewerMode])

  const takenCount = filteredHistories.filter((item) => item.status === 'taken').length
  const missedCount = filteredHistories.filter((item) => item.status === 'missed').length
  ...
  const getPatientNameTag = (history: IRES_History) => {
    ...
    return history.patientName || (carePatients.find((item) => item.patientId === history.patientId)?.patientName ?? null)
  }
        ...
        <Container>
          <View style={styles.summaryRow}>
            <View style={styles.summaryChip}>
              <Text style={styles.summaryChipText}>{filteredHistories.length} 筆紀錄</Text>
            </View>
            <View style={[styles.summaryChip, styles.takenChip]}>
              <Text style={[styles.summaryChipText, styles.takenChipText]}>{takenCount} 已服藥</Text>
            </View>
            <View style={[styles.summaryChip, styles.missedChip]}>
              <Text style={[styles.summaryChipText, styles.missedChipText]}>{missedCount} Missed</Text>
            </View>
          </View>
          <View style={styles.list}>
            {filteredHistories.length > 0 ? filteredHistories.map((history) => {
              const icon = evaluateDosageFormIcon({ dosageForm: history.medicationDosageForm })
              const statusText = history.status === 'taken' ? '已服藥' : 'Missed'
              ...
              return (
                <Pressable
                  key={history.id}
                  style={styles.card}
                  onPress={() => { router.push(`/history/${history.id}`) }}
                >
                  <View style={styles.cardHeader}>
                    ...
                    <View style={styles.content}>
                      <View style={styles.cardTitleRow}>
                        <Text style={styles.title}>{history.medicationName}</Text>
                        <Text style={[styles.statusText, { color: statusColor }]}>{statusText}</Text>
                      </View>
                      <Text style={styles.meta}>
                        預定時間 {dayjs(history.scheduledTime).format('HH:mm')}
                      </Text>
                      <Text style={styles.meta}>
                  ...
                  {patientNameTag ? (
                    <View style={styles.cardFooter}>
                      <View style={styles.patientTag}>
                        <Text style={styles.patientTagText}>{patientNameTag}</Text>
                      </View>
                    </View>
                  ) : null}
                </Pressable>
              ...
              <View style={styles.emptyState}>
                <ThemedText type="subtitle">這一天沒有紀錄</ThemedText>
                <ThemedText style={styles.emptyText}>換一天看看，這裡會顯示已服藥與 missed 的結果。</ThemedText>
              </View>
            )}
          </View>
        </Container>
