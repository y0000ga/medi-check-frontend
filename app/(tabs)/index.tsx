...
const Screen = () => {
  ...
  const filteredEvents = useMemo(() => {
    if (viewerMode === 'self') {
      return events.filter((event) => event.patientId === selfPatient?.id)
    }

    if (selectedPatientId) {
      return events.filter((event) => event.patientId === selectedPatientId)
    }

    const carePatientIds = new Set(carePatients.map((item) => item.patientId))
    return events.filter((event) => carePatientIds.has(event.patientId))
  }, [carePatients, events, selectedPatientId, selfPatient?.id, viewerMode])
  ...
  const getPatientNameTag = (event: IRES_Event) => {
    ...
    return event.patientName || (carePatients.find((item) => item.patientId === event.patientId)?.patientName ?? null)
  }
    ...
    setEvents((prev) =>
      prev.map((item) =>
        event.id === item.id
          ? {
              ...item,
              intakenTime: item.intakenTime ? null : dayjs().toISOString(),
            }
          : item
      )
    )
    ...
    <>
      ...
      <ThemedView style={styles.container}>
        <Header>
          ...
          <ThemedText type="subtitle" style={styles.subtitle}>{dayjs().format('dddd, MMM DD')}</ThemedText>
        </Header>
        <Container>
          <View style={styles.overview}>
            <View style={styles.chip}>
              <Text style={styles.chipText}>{filteredEvents.length} REMAINING</Text>
            </View>
          </View>
          <View style={styles.eventContainer}>
            {filteredEvents.map((event) => (
              <EventCard
                ...
                onViewMedication={() => router.push(`/medication/info?id=${event.medicationId}`)}
                onManage={() => router.push(`/schedule/info?id=${event.scheduleId}`)}
