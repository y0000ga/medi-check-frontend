...
import { fetchHistoryDetail, updateHistory } from "@/libs/api/history";
import { HistorySource, HistoryStatus, IRES_History } from "@/types/mock";
import { evaluateDosageFormIcon } from "@/utils/common";
import { DOSE_UNIT_LABELS } from "@/constants/medication";
import { COMMON_SYMPTOM_TAGS, EMPTY_HISTORY_FORM, SOURCE_OPTIONS, STATUS_OPTIONS } from "@/constants/history";
import { EditableHistoryValues } from "@/types/history";
import { formatDateTime, getRecommendedSource, splitCustomTags, toFormValues } from "@/utils/history";
...
const HistoryDetailModal = () => {
  ...
  const [form, setForm] = useState<EditableHistoryValues>(EMPTY_HISTORY_FORM);
  ...
  const icon = evaluateDosageFormIcon({ dosageForm: history?.medicationDosageForm ?? null });
  ...
  const handleSave = async () => {
    ...
    const symptomTags = [...form.symptomTags, ...splitCustomTags(form.customSymptomTagsText)]
      .filter(Boolean)
    ...
    try {
      const updated = await updateHistory(history.id, {
        ...
        takenAmount: form.takenAmount === "" ? null : Number(form.takenAmount),
            ...
            history ? (
              isEditing ? (
                <Pressable onPress={handleCancel}>
                  <ThemedText style={styles.headerAction}>取消</ThemedText>
                </Pressable>
              ) : (
        ...
        <Container>
          {history ? (
            <>
              <View style={styles.heroCard}>
                <View style={[styles.heroIcon, { backgroundColor: icon.backgroundColor }]}>
                  <IconSymbol size={26} name={icon.name} color={icon.color} />
                </View>
                <View style={styles.heroContent}>
                  <ThemedText type="subtitle">{history.medicationName}</ThemedText>
                  <ThemedText style={styles.heroMeta}>{HistoryStatus[history.status]}</ThemedText>
                </View>
              </View>

              <SectionCard title="固定快照">
                <InfoRow label="藥物名稱" value={history.medicationName} />
                <InfoRow label="排程劑量" value={amountLabel || "未提供"} />
                <InfoRow label="預定時間" value={formatDateTime(history.scheduledTime)} />
                <InfoRow label="藥物 ID" value={history.medicationId} />
                <InfoRow label="提醒規則 ID" value={history.scheduleId} />
                <InfoRow label="藥物名稱" value={history.medicationName} />
                <InfoRow label="藥物名稱" value={history.medicationName} />
                <InfoRow label="藥物名稱" value={history.medicationName} />
                <InfoRow label="藥物名稱" value={history.medicationName} />
                ...
                {isEditing ? (
                  <>
                    ...
                    <FieldInput
                      ...
                      onChangeText={(intakenTime) => setForm((current) => ({ ...current, intakenTime }))}
                      placeholder="例如：2026-04-03T08:15:00.000Z"
                    />
                    <FieldInput
                      label="實際服用量"
                      value={form.takenAmount}
                      onChangeText={(takenAmount) => setForm((current) => ({ ...current, takenAmount }))}
                    ...
                    <FieldInput
                      label="原因"
                      value={form.reason}
                      onChangeText={(reason) => setForm((current) => ({ ...current, reason }))}
                    ...
                    <View style={styles.fieldGroup}>
                      <ThemedText style={styles.fieldLabel}>症狀標籤</ThemedText>
                      <ThemedText style={styles.helperText}>點選常用標籤，或在下方補充自訂標籤。</ThemedText>
                    ...
                    <FieldInput
                      ...
                      onChangeText={(customSymptomTagsText) =>
                        setForm((current) => ({ ...current, customSymptomTagsText }))
                      }
                    ...
                    <FieldInput
                      label="備註"
                      value={form.memo}
                      onChangeText={(memo) => setForm((current) => ({ ...current, memo }))}
                    ...
                    <FieldInput
                      label="服藥後感受"
                      value={form.feeling}
                      onChangeText={(feeling) => setForm((current) => ({ ...current, feeling }))}
                  ...
                  <>
                    ...
                    <InfoRow
                      label="實際服用量"
                      value={history.takenAmount != null
                        ? `${history.takenAmount} ${history.doseUnit ? DOSE_UNIT_LABELS[history.doseUnit] : ""}`.trim()
                        : "未填寫"}
                    />
                    <InfoRow label="評分" value={history.rate?.toString() ?? "未填寫"} />
                    <InfoRow label="原因" value={history.reason || "未填寫"} />
                    <InfoRow label="症狀標籤" value={displaySymptomTags.length > 0 ? displaySymptomTags.join("、") : "未填寫"} />
                    <InfoRow label="備註" value={history.memo || "未填寫"} />
                    <InfoRow label="服藥後感受" value={history.feeling || "未填寫"} />
                  </>
                )}
              </SectionCard>
            </>
            ...
            <View style={styles.emptyState}>
              <ThemedText type="subtitle">找不到這筆歷史紀錄</ThemedText>
              <ThemedText style={styles.emptyText}>
                這筆資料可能已被移除，或目前的 mock 資料中沒有對應內容。
              </ThemedText>
            </View>
          )}
        </Container>
        {history && isEditing ? (
          <Header>
            <Pressable style={styles.saveButton} onPress={handleSave}>
              <ThemedText style={styles.saveButtonText}>儲存紀錄</ThemedText>
            </Pressable>
          </Header>
        ) : null}
      </ThemedView>
