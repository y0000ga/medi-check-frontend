...
const MedicationCard = ({
    ...
    return (
        <Pressable onPress={() => { router.push(`/medication/info?id=${id}`) }}>
            <View style={styles.card}>
                <View style={styles.header}>
                    <View style={[styles.iconContainer, { backgroundColor: icon.backgroundColor }]}>
                        <IconSymbol size={24} name={icon.name} color={icon.color} />
                    </View>
                    <View style={styles.content}>
                        <Text style={styles.title}>{name}</Text>
                        <Text style={styles.meta}>{MEDICATION_DOSAGE_FORM[dosageForm]}</Text>
                    </View>
