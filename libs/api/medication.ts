...
let mockMedications: IRES_Medication[] = RES_MEDICATIONS.map((item) => ({
  ...item,
  patientId: DB_MEDICATIONS.find((medication) => medication.id === item.id)?.patientId ?? '',
}));

export async function fetchMedications({ search }: { search?: string }) {
  ...
  return mockMedications.filter(item => {
    ...
    const isExistedSearch = keyword
      ? item.name.toLowerCase().includes(keyword) || item.memo.toLowerCase().includes(keyword)
      : true
    return isExistedSearch
  })
}

export async function fetchMedicationDetail(id: string) {
  // return request<IMedication>(`/medications/${id}`);
  return mockMedications.find(item => item.id === id)
}

export async function fetchMedicationPatientMap() {
...
export async function fetchMedicationsByPatient(patientId: string) {
  const patientMap = await fetchMedicationPatientMap()
  const targetMedicationIds = patientMap
    .filter((item) => item.patientId === patientId)
    .map((item) => item.medicationId)

  return mockMedications.filter((item) => targetMedicationIds.includes(item.id))
}

export async function createMedication(payload: Omit<IRES_Medication, 'id'>) {
  // return request<IMedication>('/medications', {
  //   method: 'POST',
  //   body: JSON.stringify(payload),
  // });

  const createdMedication = { ...payload, memo: payload.memo ?? '', id: dayjs().toString() }
...
export async function updateMedication(
  ...
  const existedMedication = mockMedications.find(item => item.id === id)

  if (!existedMedication) {
    throw new Error('Medication not found');
  }

  const updatedMedication = { ...existedMedication, ...payload, id } as IRES_Medication
